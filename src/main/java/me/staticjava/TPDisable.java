 ...
 */
public class TPDisable implements CommandExecutor {

    private WorldGuardTP plugin;

    public TPDisable(WorldGuardTP plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
        ...
        if (player.hasPermission("wgtp.disable")) {
            if (args.length == 1) {
                String id = args[0];
                ...
                if (protectedRegion != null) {
                    if (!plugin.getConfig().getStringList("disabledRegions").contains(id + ";" + world)) {
                        ...
                        player.sendMessage(ChatColor.GOLD + "[" + ChatColor.AQUA + "WGTP" + ChatColor.GOLD + "] " + ChatColor.GREEN + "Successfully disabled TP in the region \"" + id + "\"!");
                        return true;
                    } else {
                        player.sendMessage(ChatColor.GOLD + "[" + ChatColor.AQUA + "WGTP" + ChatColor.GOLD + "] " + ChatColor.RED + "TP in this region was already disabled!");
                        return true;
                    }
                } else {
                ...
                player.sendMessage(ChatColor.GOLD + "[" + ChatColor.AQUA + "WGTP" + ChatColor.GOLD + "] " + ChatColor.RED + "Invalid usage! Type /tpdisable <regionName>.");
                return true;
            }
        } else {
